...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      addAlert(err.response?.data?.detail || 'Invalid credentials. Please try again.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-950 px-4 py-12 relative overflow-hidden transition-colors duration-200">
      ...
      <div className="absolute top-[20%] right-[-10%] w-96 h-96 bg-purple-400 rounded-full mix-blend-multiply filter blur-3xl opacity-20 dark:opacity-10 animate-blob animation-delay-2000"></div>
      ...
      <div className="max-w-md w-full glass z-10 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl p-8 rounded-3xl shadow-2xl border border-white/20 dark:border-gray-800">
        <div className="text-center mb-10">
          ...
          <p className="mt-3 text-gray-500 dark:text-gray-400 text-sm">Sign in to your account and continue your journey</p>
        </div>
